    ...
    <>
      ...
      <div className='flex justify-center sticky top-1 z-10'>
        <div className='relative'>
          <input className='w-[450px] h-[45px] text-xl pl-2 border border-black rounded-xl' value={text} onChange={(e) => setText(e.target.value)} />
          ...
          {text && <div className='absolute bg-white w-[600px] border border-t-gray-400 rounded-lg p-1'>
            <ul className='cursor-pointer'>
              {suggestions.map((suggest) => <Link key={suggest._id} to={"/search?q=" + text}><li className='h-[30px] rounded-lg hover:bg-gray-300 p-1' >{suggest.name}</li></Link>)}
            </ul>
          </div>}
