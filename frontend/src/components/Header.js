...
const Header = () => {
  ...
  return (
       ...
       <div className='w-full flex justify-around sticky top-0 z-10 p-2 bg-white rounded-lg shadow-xl'>
         ...
         <div><NavBar/></div>
         <div>{localStorage.getItem('userInfo')?<div className='text-blue-600 font-semibold mt-2'>{decoded.email}</div>: <Link to='/login'><div><button className='w-[135px] rounded-lg shadow-lg hover:bg-blue-800 h-[35px] text-white bg-blue-600 p-1 m-1'>Login</button></div></Link>}</div>
